...
int main() {
    ...
    // 先检查是否存在cookie文件(每次用户登录，服务器都会回发一个cookie)
    ...
    if (file.good()) {
        ...
        if (ans_str != "NULL") {    // 服务器查询cookie合法，is_login
            is_login = true;
            login_name = ans_str;
        }
    }    
    while (!is_login) {
        ...
        if (choice == 1) {
            ...
            if (recv_str.substr(0,2) == "ok") {
                ...
                system(gen_cookie_cmd.c_str()); // system 函数执行操作系统的命令
    ...
    while (is_login) {
        ...
        if (choice == 1) {
            ...
            // TODO 接收服务器回复，判断聊天是否合法
            cout << "请输入你想说的话(输入exit退出)：" << endl;

            // 3 创建两个线程，分别用于接收数据和发送数据
