...
HttpConnection* HttpConnection::initConnPool(int MAX_SIZE) {
    conns_pool = new HttpConnection[MAX_SIZE]; // 创建连接数组
    return conns_pool;
}
...
/*初始化 根据 accept得到的客户端fd和addr 初始化http连接*/
void HttpConnection::init(int sock_fd, const sockaddr_in& addr) {
    ...
    // 添加至epoll监听，客户端fd设置 EPOLLONESHOT
...
bool HttpConnection::login() {
    ...
    // 将 登录状态 和 session_id(作为cookie) 发回客户端
